...
return '⚔️ آماده پشتیبانی از مردم این قلمرو هستم  گلادیاتور عرفان⚔️'
  ...
  save_data(_config.moderation.data, data)
      ...
      save_data(_config.moderation.data, data)
    if not lang then
  return '*🎭 درود بر گلادیاتور عرفان هرکولس 🎭*'
else
...
if matches[2] == "pin" and is_owner(msg) or matches[2] == "وبسایت" then
return unlock_pin(msg, data, target)
end
end
if matches[1] == "mute" and is_mod(msg) or matches[1] == "ممنوعیت" and is_mod(msg) then
local target = msg.to.id
if matches[2] == "all" or matches[2] == "همه چیز" then
...
if matches[2] == "document" or matches[2] == "اسناد" then
return mute_document(msg ,data, target)
end
if matches[2] == "tgservice" or matches[2] == "ورودخروج" then
...
if matches[1] == "unmute" and is_mod(msg) or matches[1] == "رفع ممنوعیت" and is_mod(msg) then
local target = msg.to.id
if matches[2] == "all" or matches[2] == "همه چیز" then
...
*!mute* `[gif | photo | document | sticker | keyboard | video | text | forward | location | audio | voice | contact | all]`
_در صورت بیصدد بودن فعالیت ها, ربات آنهارا ذف نخواهد کرد_
